...
contract('MarketplaceListing', function (accounts) {
    ...
    describe('Should pass OK marketplace journeys', () => {
        it('create OK ERC721 listing for ERC20 asset with cashback from custodial wallet', async function () {
            ...
            const b = await marketplace.buyAssetFromListingForExternalBuyer('1', erc20.address, buyer, { from: seller });
        ...
        it('create OK ERC1155 listing for native asset', async function () {
            ...
            await token.setApprovalForAll(marketplace.address,true, { from: seller1155});
            let listings = await marketplace.getListing('1');
            expect(listings[0]).to.be.equal('1');
            expect(listings[2]).to.be.equal('0');
